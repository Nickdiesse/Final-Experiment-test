...
cursor.execute("""
    SELECT 
        AVG(llama_time) / 1000.0 AS avg_response_time_llama_sec,
        AVG(orca_time) / 1000.0 AS avg_response_time_orca_sec,
        AVG(falcon_time) / 1000.0 AS avg_response_time_falcon_sec
    FROM esempione_ontologia_db;
""")
