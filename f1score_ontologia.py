...
cursor.execute("SELECT id, ground_truth, llama_answer, orca_answer, falcon_answer FROM esempione_ontologia_db;")
...
for row in rows:
    ...
    f1_llama = calculate_f1(answers["llama"], ground_truth)
    f1_orca = calculate_f1(answers["orca"], ground_truth)
    f1_falcon = calculate_f1(answers["falcon"], ground_truth)

    # Inserisci i risultati nella tabella model_evaluation
    cursor.execute("""
        INSERT INTO f1score_ontologia (id, f1_llama, f1_orca, f1_falcon)
        VALUES (%s, %s, %s, %s);
    """, (id, f1_llama, f1_orca, f1_falcon))
