...
cursor.execute("""
    SELECT 
        AVG(time_llama) / 1000.0 AS avg_response_time_llama_sec,
        AVG(time_orca) / 1000.0 AS avg_response_time_orca_sec,
        AVG(time_falcon) / 1000.0 AS avg_response_time_falcon_sec
    FROM esempione_prompt_db;
""")
